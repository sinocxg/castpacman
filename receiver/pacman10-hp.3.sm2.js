google.pacman || function () {
    ...
    g.playSound = function (b, c, d) {
		console.debug("**** playSound " + b + g.soundAvailable + '' + google.pacManSound + "" + g.paused);
	...
	g.playSoundLoop = function ( b, c, opt ){
		function loopSound(d, opt) {
			d.play({
				onfinish: function() {
				  loopSound(d, opt);
				},
    ...
    g.prepareSound = function () {
		...
		soundManager.setup({
		  url: 'sm2/swf/',
		  onready: function() {
			var snd_a = [ "eating-dot-2", "ambient-1", "death-double", "fruit", 
							"eating-dot-1", "death", "ambient-3", "ambient-fright", 
							"ambient-eyes", "ambient-2", "extra-life", "start-music-double", 
							"eating-dot-double", "start-music", "eating-ghost", "ambient-4", "cutscene" ];
			for(var si = 0; si < snd_a.length; si++){
				g.sound[snd_a[si]] = soundManager.createSound({
					id: snd_a[si],
					url: 'sounds/' + snd_a[si] + '.mp3',
					autoLoad: true,
					autoPlay: false,
					onload: function() {
						//console.debug('The sound '+this.id+' loaded!');
					},
					volume: 50
				});
			}
			g.sound.length = si;
			window.setTimeout(g.soundLoaded(snd_a.length), 3E3);
		  },
		  ontimeout: function() {
			console.debug('SM2 could not start.');
